...
if [ -f "$HOLD_FILE" ]; then 
    notify-send --urgency=normal --category=im.receieved --icon=help-info-symbolic "DermoDex Hold Still Active." "DermoDeX seems to be having trouble removing your hold file, go to ~/.local/share/dermodex and manually remove the file dermodex_hold."
else
